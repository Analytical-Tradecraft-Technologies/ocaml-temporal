...
| --- | --- | --- | --- |
| `verify` | `make verify OCAML_VERSION=<matrix version>` | `make verify OCAML_VERSION=5.2` (or another locally available image) | Docker-backed OCaml build/lint, Rust tests, bridge/install tests, and repository quality contracts. PRs use the representative cells; scheduled runs use the exhaustive matrix. |
