...
(** A defect after suspension fails the task and discards its cache generation.
    Stale work cannot resume it; replay must rebuild the same run with a fresh
    command sequence while leaving the durable workflow open. *)
let test_resumed_defect_discards_run () =
  let supervisor = fake_supervisor () in
  let activity =
  ...
  | [], Some _ -> ()
  | _ -> failwith "invalid timer resolution did not fail only the workflow task"
  end;
  enqueue supervisor
    (activation ~run_id:"run-resumed-failure"
       [ Protocol.Fire_timer { seq = timer_seq } ]);
  ...
  | Ok (Adapter.Rejected { error; lease_retired = true; _ })
    when String.equal error.code "unknown_run_id" -> ()
  | _ -> failwith "stale timer resumed the discarded cache generation");
  if (latest_completion supervisor).commands <> [] then
    failwith "discarded execution emitted another workflow command";
  enqueue supervisor
    (activation ~run_id:"run-resumed-failure"
       [ initialize ~run_id:"run-resumed-failure"
           ~workflow_type:"native_worker_resumed_failure" ]);
  expect_completed ~terminal:false (Result.get_ok (Worker.poll worker));
  (match (latest_completion supervisor).commands with
  | [ Protocol.Start_timer { seq; _ } ] when seq = timer_seq -> ()
  | _ -> failwith "replay did not reconstruct the original timer sequence");
  if Option.is_some (latest_completion supervisor).task_failure then
    failwith "replayed generation retained the previous task failure";
  if Hashtbl.length supervisor.leased <> 0 then
    failwith "resumed defect or replay leaked a native lease";
...
let test_duplicate_child_terminal_while_parent_pending () =
  ...
  (* Task failure discarded the defective cache generation and its pending
     timer. A stale activation must retire its lease without resuming it. *)
  enqueue supervisor
    (activation ~run_id [ Protocol.Fire_timer { seq = timer_seq } ]);
  (match Worker.poll worker with
  | Ok (Adapter.Rejected { error; lease_retired = true; _ })
    when String.equal error.code "unknown_run_id" -> ()
  | _ -> failwith "late timer was accepted for a discarded parent");
  if (latest_completion supervisor).commands <> [] then
...
let () =
  ...
  test_resumed_defect_discards_run ();
